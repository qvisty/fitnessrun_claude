...
<div class="contestantLaps form large-9 medium-8 columns content">
    <?php
        ...
        echo $this->Form->label('activerace',__('Races'));
        echo $this->Form->input('activerace', ['id'=>'activerace','options' => $races,'label'=>false]);
        echo $this->Form->label('autosubmit',__('Auto submit'));
        echo $this->Form->input('autosubmit', ['id'=>'autosubmit','type' => 'checkbox','hiddenField'=>false,'label'=>false]);
        echo $this->Form->end();
    ?>
    ...
    <fieldset>
        ...
        <?php
            ...
            echo '<div id="nocontestantfound">'.__("No contestant found with barcode").'</div>';
...
<script>
    ...
    {
        if(typeof getUrlParameter("autosubmit") == 'undefined')
        {
        ...
        $("#autosubmit").prop( "checked", autosubmit ); 
    }
    ...
    $(document).keypress(function(e) {
        ...
        if(e.which == 13) {
            ...
            {
                $("#nocontestantfound").show();
                $("#barcode").val()
                $("#nocontestantfound").text("<?php echo __("No contestant found with barcode"); ?>"+": "+$("#barcode").val());
            }
            $("#barcode").val("");
            $("#barcode").focus(); 
        }
