...
class RacesController extends AppController
{
    ...
    public function view($id = null)
    {
        $race = $this->Races->get($id, [
            'contain' => ['ContestantFinishtimes', 'ContestantLaps', 'RaceContestants']
        ]);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Races->delete($race)) {
            ...
            //Sletter alle relaterede data - skal kun udføres hvis løbet er blevet slettet. 
            
            //Sletter alle omgange der findes til det pågældende løb
            $laps = $this->ContestantLaps->find('all')->where(['race_id'=>$id]);
            foreach($laps as $lap)
            {
                $this->ContestantLaps->delete($lap);
            }

            //Sletter alle sluttider
            $finishtimes = $this->ContestantFinishtimes->find('all')->where(['race_id'=>$id]);
            foreach($finishtimes as $finishtime)
            {
                $this->ContestantFinishtimes->delete($finishtime);
            } 
